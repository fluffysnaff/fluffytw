...
void CGameClient::OnConsoleInit()
{
	...
	m_Input.Add(&m_Chat); // chat has higher prio due to that you can quit it by pressing esc
	...
	// Add your hacks here:
	m_Hacks.push_back(new HBalance(this)); // this = m_pClient
	m_Hacks.push_back(new HAimbot(this)); // this = m_pClient

	// console init the hacks
	for(int i = 0; i < m_Hacks.size(); i++)
		m_Hacks[i]->OnConsoleInit();
...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker, bool IsDummy)
{
	...
	for(int i = 0; i < m_Hacks.size(); i++)
		m_Hacks[i]->OnMessage(MsgId, pUnpacker, IsDummy);
}
...
void CGameClient::OnStateChange(int NewState, int OldState)
{
	...
	for(int i = 0; i < m_Hacks.size(); i++)
		m_Hacks[i]->OnStateChange(NewState, OldState);
}
...
void CGameClient::OnRconType(bool UsernameReq)
{
	...
	for(int i = 0; i < m_Hacks.size(); i++)
		m_Hacks[i]->OnRconType(UsernameReq);
}
